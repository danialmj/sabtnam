...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(!isset($_SESSION['captcha']) || strtolower($captcha_input) !== strtolower($_SESSION['captcha'])) {
        $message = "<h3 style='color:red;'>❌ کد امنیتی (کپچا) اشتباه است</h3>";
    } else {
        // اتصال به دیتابیس
        ...
        $stmt = $conn->prepare("SELECT user_name, pas FROM baty WHERE user_name = ? AND pas = ?");
        if (!$stmt) {
            die("❌ خطا در prepare: " . $conn->error);
        }

        $stmt->bind_param("ss", $username, $password); // اصلاح bind_param
        $stmt->execute();
